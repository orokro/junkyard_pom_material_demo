 ...
 * arena/seed.js
 * ----------------------------------------------------------------------------
 * Deterministic seeding utilities for the Arena POC.
 *
 * A single seed string drives arena generation. Everything that needs
 * randomness must derive from a PRNG created here — no bare Math.random in
 * generation, so every client reproduces the same arena from the same seed.
 *
 ...
 *
 * Self-contained (a sibling of the junkyard's seed.js) so the Arena POC never
 * depends on junkyard code. The last-used seed persists under an arena-specific
 * localStorage key.
 * ============================================================================
 */

const STORAGE_KEY = "arena_poc.lastSeed";
 ...
 * @param {string} [salt] Extra namespacing (e.g. "layout", "props").
