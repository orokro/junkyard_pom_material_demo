...
export function generateTires(dims, model) {
	...
	// Bridge deck cells are open beneath, so they autotile like normal ground (their
	// wall-facing edges get normal straights). The dedicated BridgeBase piece is
	// added only at the open DROP-OFF short-end, which the autotiler leaves bare
	// (open neighbour → no straight), so the two never collide on the same edge.
